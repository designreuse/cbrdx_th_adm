...
public interface VDotacionesRefactorRepository extends CrudRepository<VDotaciones, Integer> {
    ...
    @Query("SELECT D FROM VDotaciones D WHERE D.indicadorHabilitado = 1 AND D.idGrupoDotacion IN (SELECT P.idGrupoDotacion FROM ProyeccionDotacion P WHERE P.idProyeccionDotacion = ?1)")
