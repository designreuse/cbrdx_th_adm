...
public interface CargosRelacionadosRefactorRepository extends CrudRepository<CargosRelacionados, Integer> {
    ...
    @Query("select u from CargosRelacionados u where indicadorHabilitado=1 and idCargo = ?2 and idTipoRelacion in (select idListaTipoCargoRelacion from ListasTiposCargosRelaciones where codigo like concat('%',?1,'%'))")
